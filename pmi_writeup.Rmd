...
The first was to run a naive bayes model on the training set. This produced poor results. 

The second was to use a High Dimensional Discriminant Analysis (hdda) approach to the data set. This produced 97% accuracy and was efficient in terms of processing time. 

Finally I ran the training using RF option. This was extremely exoensive in processing time, so I abandoned it. 
...
Fit_nb <- train(classe ~accel_arm_x_range+accel_arm_y_range+accel_belt_z_range, method = "nb", preProcess = c("center", "scale"), data = training)
...
Fit <- train(classe ~accel_arm_x+accel_arm_y+accel_arm_z+accel_belt_x+accel_belt_y+accel_belt_z, method = "rpart", data = training1)
##Fit <- train(classe ~ ., method = 'rf', data = training)


Predictions <- predict(Fit, newdata = testing1)
# 
